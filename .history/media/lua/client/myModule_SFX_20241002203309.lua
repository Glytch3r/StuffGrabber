...
function myModule.VolumeHandler(dist, range)
    if dist >= range then
        return 0
    elseif dist <= 0 then
        return 1
...
function myModule.getCrySfx()
	return 'PsychoCry_'..tostring(ZombRand(1,7))
end
function myModule.getScreamSfx()
	return 'PsychoScream_'..tostring(ZombRand(1,3))
end
function myModule.getLaughSfx()
	return 'PsychoLaugh_'..tostring(ZombRand(1,8))
end

function myModule.playSFX(zed, sfx)
	if not myModule.ismyModule(zed) then return end
	local range = 50
	local pl = getPlayer(); if not pl then return end
	if myModule.isClosestPl(pl, zed) then
		local sq = zed:getSquare()
		if sq then
			local vol = myModule.VolumeHandler(myModule.checkDist(pl, zed), range)
			if luautils.stringStarts(sfx, "PsychoScream") or string.find(sfx, "PsychoScream")  then
				if SandboxVars.myModule.DeathScreamAttract then
...
function myModule.preSFX(zed)

	if not myModule.ismyModule(zed) then return end
	local pl = getPlayer(); if not pl then return end
	if zed:getEmitter() then
		local audioName = nil
		local list = myModule.sfxBanList
		for _, audioName in ipairs(list) do
			if zed:getEmitter():isPlaying(audioName) then
				...
				if myModule.doRoll(SandboxVars.myModule.SFXRate) then
					if myModule.isDay() and myModule.isCrying(zed) then
						if myModule.isTarg(pl, zed) then
							myModule.playSFX(zed, myModule.getScreamSfx())
						else
							myModule.playSFX(zed, myModule.getCrySfx())
						end
					end

					if myModule.isNight() and myModule.isLaughing(zed) then
						myModule.playSFX(zed, myModule.getLaughSfx())
					end
				end
...
Events.OnZombieUpdate.Remove(myModule.preSFX)
Events.OnZombieUpdate.Add(myModule.preSFX)

function myModule.playPlSfx(pl, sfx, attract)
	if not myModule.isWearingmyModule(pl) then return end
...
function myModule.keypress(key)
	local pl = getPlayer()

    if key == getCore():getKey("Shout") then
		if myModule.isWearingmyModule(pl)   then
			if pl:isCanShout() then
				pl:setCanShout(false);
			end
			local sfx = nil
			if pl:isSneaking() then
				sfx = myModule.getCrySfx()
				myModule.playPlSfx(pl, sfx, false)
			elseif  pl:IsAiming()  then
				sfx = myModule.getScreamSfx()
				myModule.playPlSfx(pl, sfx, true)
			else
				sfx = myModule.getLaughSfx()
				myModule.playPlSfx(pl, sfx, false)
			end
...
Events.OnKeyPressed.Remove(myModule.keypress)
Events.OnKeyPressed.Add(myModule.keypress)
