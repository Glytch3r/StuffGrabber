...
function StuffGrabber_Act:DropStuff(pl, dest, toDrop) -- self:DropLogs()
    ...
    for i = 1, inv:getItems():size()  do
        ...
        if item and item:getFullType() == toDrop then
            ...
            count = count + 1
    --[[         pl:getCurrentSquare():AddWorldInventoryItem(item, 0, 0, 0)
            inv:Remove(item) ]]
            ISTimedActionQueue.add(ISDropItemAction:new(pl, item))
            --ISTimedActionQueue.add(StuffGrabber_Drop:new(pl, item, pl:getCurrentSquare(), ZombRand(2), ZombRand(2), 0, 0, true, 50))
        end
    end
