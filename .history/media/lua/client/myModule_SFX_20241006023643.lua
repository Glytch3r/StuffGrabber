...
function StuffGrabber.VolumeHandler(dist, range)
    if dist >= range then
        return 0
    elseif dist <= 0 then
        return 1
...
function StuffGrabber.getCrySfx()
	return 'PsychoCry_'..tostring(ZombRand(1,7))
end
function StuffGrabber.getScreamSfx()
	return 'PsychoScream_'..tostring(ZombRand(1,3))
end
function StuffGrabber.getLaughSfx()
	return 'PsychoLaugh_'..tostring(ZombRand(1,8))
end

function StuffGrabber.playSFX(zed, sfx)
	if not StuffGrabber.isStuffGrabber(zed) then return end
	local range = 50
	local pl = getPlayer(); if not pl then return end
	if StuffGrabber.isClosestPl(pl, zed) then
		local sq = zed:getSquare()
		if sq then
			local vol = StuffGrabber.VolumeHandler(StuffGrabber.checkDist(pl, zed), range)
			if luautils.stringStarts(sfx, "PsychoScream") or string.find(sfx, "PsychoScream")  then
				if SandboxVars.StuffGrabber.DeathScreamAttract then
...
function StuffGrabber.preSFX(zed)

	if not StuffGrabber.isStuffGrabber(zed) then return end
	local pl = getPlayer(); if not pl then return end
	if zed:getEmitter() then
		local audioName = nil
		local list = StuffGrabber.sfxBanList
		for _, audioName in ipairs(list) do
			if zed:getEmitter():isPlaying(audioName) then
				...
				if StuffGrabber.doRoll(SandboxVars.StuffGrabber.SFXRate) then
					if StuffGrabber.isDay() and StuffGrabber.isCrying(zed) then
						if StuffGrabber.isTarg(pl, zed) then
							StuffGrabber.playSFX(zed, StuffGrabber.getScreamSfx())
						else
							StuffGrabber.playSFX(zed, StuffGrabber.getCrySfx())
						end
					end

					if StuffGrabber.isNight() and StuffGrabber.isLaughing(zed) then
						StuffGrabber.playSFX(zed, StuffGrabber.getLaughSfx())
					end
				end
...
Events.OnZombieUpdate.Remove(StuffGrabber.preSFX)
Events.OnZombieUpdate.Add(StuffGrabber.preSFX)

function StuffGrabber.playPlSfx(pl, sfx, attract)
	if not StuffGrabber.isWearingStuffGrabber(pl) then return end
...
function StuffGrabber.keypress(key)
	local pl = getPlayer()

    if key == getCore():getKey("Shout") then
		if StuffGrabber.isWearingStuffGrabber(pl)   then
			if pl:isCanShout() then
				pl:setCanShout(false);
			end
			local sfx = nil
			if pl:isSneaking() then
				sfx = StuffGrabber.getCrySfx()
				StuffGrabber.playPlSfx(pl, sfx, false)
			elseif  pl:IsAiming()  then
				sfx = StuffGrabber.getScreamSfx()
				StuffGrabber.playPlSfx(pl, sfx, true)
			else
				sfx = StuffGrabber.getLaughSfx()
				StuffGrabber.playPlSfx(pl, sfx, false)
			end

		else
			if not pl:isCanShout() then
				pl:setCanShout(true);
			end
		end
...
Events.OnKeyPressed.Remove(StuffGrabber.keypress)
Events.OnKeyPressed.Add(StuffGrabber.keypress)
