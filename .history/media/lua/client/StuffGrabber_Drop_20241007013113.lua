...
StuffGrabber_Drop = ISBaseTimedAction:derive("StuffGrabber_Drop");

function StuffGrabber_Drop:isValid()
...
function StuffGrabber_Drop:update()
	self.item:setJobDelta(self:getJobDelta());
end

function StuffGrabber_Drop:start()
...
function StuffGrabber_Drop:stop()
    self.item:setJobDelta(0.0);
    self.character:stopOrTriggerSound(self.sound)
    ISBaseTimedAction.stop(self);
end

function StuffGrabber_Drop:perform()
	if self.sound then
		local actionQueue = ISTimedActionQueue.getTimedActionQueue(self.character)
		local nextAction = actionQueue.queue[2]
		if not nextAction or (nextAction.Type ~= StuffGrabber_Drop.Type) or (nextAction.item:getFullType() ~= self.item:getFullType()) then
			self.character:stopOrTriggerSound(self.sound)
		else
			nextAction.sound = self.sound
		end
	end
...
function StuffGrabber_Drop:new(character, item, sq, xoffset, yoffset, zoffset, rotation, isMultiple, maxTime)
	local o = {}
	setmetatable(o, self)
	self.__index = self
