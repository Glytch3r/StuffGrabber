...
function myModule.isFirearm(pl, wpn)
...
function myModule.getRandPart(pl)
...
function myModule.doRoll(percent) if percent >= ZombRand(1, 101) then return true end return false end
-----------------------            ---------------------------
function myModule.checkDist(pl, zed)
	local dist = pl:DistTo(zed:getX(), zed:getY())
    return math.floor(dist)
end

function myModule.isWithinRange(pl, zed, range)
	local dist = pl:DistTo(zed:getX(), zed:getY())
    return dist <= range
end

function myModule.isClosestPl(pl, zed)
	if not myModule.ismyModule(zed) then return end
	local plDist = myModule.checkDist(pl, zed)
